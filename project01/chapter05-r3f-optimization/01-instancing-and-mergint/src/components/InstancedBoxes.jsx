...
export const InstancedBoxes = () => {
  ...
  const colors = useMemo(() => (
    new Float32Array(
      Array.from({length: boxCount}, () => color.set(colorPalettes[Math.floor(Math.random() * 5)]).toArray()).flat()
    )
  ), [])
    ...
    for(let x=0; x<spaceWidth; x+=1) {
      for(let y=0; y<spaceWidth; y+=1) {
        for(let z=0; z<spaceWidth; z+=1) {
          ...
          object3D.position.set(halfOfSpaceWidth - x + Math.random(), halfOfSpaceWidth - y + Math.random(), halfOfSpaceWidth - z + Math.random())
